...
function bindForms() {
  ...
  document.getElementById('add-player-form').addEventListener('submit', e => {
    e.preventDefault();
    addPlayer({name:document.getElementById('player-name').value,team:document.getElementById('player-team').value,position:document.getElementById('player-pos').value,rank:document.getElementById('player-rank').value,adp:document.getElementById('player-adp').value,projection:document.getElementById('player-proj').value});
...
function configureCloudDataUI(){
  ...
  if(reset){reset.id='reset-draft-data';reset.textContent='Reset draft';const card=reset.closest('.action-card');if(card){const label=card.querySelector('.section-label');const h=card.querySelector('h2');const p=card.querySelector('p');if(label)label.textContent='DRAFT RESET';if(h)h.textContent='Restart draft';if(p)p.textContent='Delete draft picks and drafted rosters so you can run the draft again. League settings, team names and player data are kept.'}}
  const head=document.querySelector('#view-backup .page-head');if(head){const eyebrow=head.querySelector('.eyebrow'),h=head.querySelector('h1'),p=head.querySelector('.page-copy');if(eyebrow)eyebrow.textContent='CLOUD DATA';if(h)h.textContent='Data & draft controls';if(p)p.textContent='Your authenticated league data is stored in Firestore. Use backup tools for portability or reset only the draft when you want to start again.'}
}

function bindControls() {
  configureCloudDataUI();
...
function maybeAutoRefresh(){if(state.picks.length)return;const last=state.feed.rankingsUpdatedAt?new Date(state.feed.rankingsUpdatedAt).getTime():0;const should=!state.players.length||!last||Date.now()-last>AUTO_RANKINGS_REFRESH_MS;if(!should)return;setTimeout(async()=>{const ok=await refreshRankings({silent:true});if(ok)await refreshSleeperMetadata({force:false,silent:true});renderAll()},500)}
function init(){bindNavigation();bindForms();bindControls();populateSettings();renderAll();maybeAutoRefresh()}
init();
