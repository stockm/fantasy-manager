...
(function installSeasonDashboard(){
  ...
  function leagueRank(){if(typeof rosterForSlot!=='function'||!(state.leagueTeams||[]).length)return{rank:null,total:0,leader:null};const rows=(state.leagueTeams||[]).map(t=>({slot:Number(t.slot),name:t.name,total:rosterForSlot(t.slot).reduce((s,p)=>s+val(p),0)})).sort((a,b)=>b.total-a.total);return{rank:rows.findIndex(x=>x.slot===Number(state.settings.draftSlot))+1,total:rows.find(x=>x.slot===Number(state.settings.draftSlot))?.total||0,leader:rows[0]||null}}
  function topFreeAgents(limit=4){const owned=new Set(Object.values(state.teamRosters||{}).flat());(state.picks||[]).forEach(p=>owned.add(p.playerId));(state.manualRosterIds||[]).forEach(id=>owned.add(id));return(state.players||[]).filter(p=>!owned.has(p.id)&&!/(OUT|IR|SUSP)/i.test(String(p.status||''))).sort((a,b)=>{const ar=num(a.rank,99999),br=num(b.rank,99999);if(ar!==br)return ar-br;return val(b)-val(a)}).slice(0,limit)}
  ...
  function renderSeason(){
    ...
    const w=week(),myRoster=roster(),cache=window.fmDerivedCache?.cacheFor?.(w)||null;
    let rank,opp,free,watch,lineup,bench;
    if(cache){
      const powers=cache.power||[],mineIndex=powers.findIndex(x=>Number(x.slot)===Number(state.settings.draftSlot));rank={rank:mineIndex>=0?mineIndex+1:null,total:mineIndex>=0?Number(powers[mineIndex].total||0):0,leader:powers[0]||null};opp=Number(cache.opponentSlot||0);free=(cache.pickups||[]).slice(0,4).map(x=>({...x.player,adjustedWeekProjection:Number(x.weekly?.adjusted||0)}));watch=(cache.rosterHealth||[]).slice(0,4);lineup=cache.myLineup||null;bench=(lineup?.bench||[]).map(id=>getPlayer(id)).filter(Boolean).sort((a,b)=>val(b)-val(a));
    }else{
      rank=leagueRank();opp=Number(state.matchups?.[String(w)]||0);free=topFreeAgents();watch=watchList();try{if(typeof window.bestWeeklyLineup==='function')lineup=window.bestWeeklyLineup(myRoster,w)}catch(e){}bench=(lineup?.bench||[]).slice().sort((a,b)=>val(b)-val(a));
    }
    ...
    root.innerHTML=`
      <div class="season-hero"><div><div class="eyebrow">SEASON COMMAND CENTER · WEEK ${w}</div><h1>${esc(teamName())}</h1><p>Manage your lineup, matchup, waivers and trades from one team-first dashboard.${cache?` <span class="muted">${esc(window.fmDerivedCache.ageText(cache))}.</span>`:''}</p></div><div class="season-hero-actions"><button class="btn secondary" data-season-go="roster">My Roster</button><button class="btn primary" data-season-go="matchups">Analyze Week ${w}</button></div></div>
      ...
      <div class="season-main-grid">
        ...
        <article class="surface season-feature"><div class="season-card-head"><div><span class="section-label">ROSTER HEALTH</span><h2>${watch.length?`${watch.length} player${watch.length===1?'':'s'} to watch`:'Roster looks healthy'}</h2></div><button class="btn secondary small" data-season-go="roster">Review roster →</button></div><div class="season-list">${watch.length?watch.map(p=>`<div><span><b>${esc(p.name)}</b><small>${esc(p.position||primaryPos(p))} · ${esc(p.team||'')}</small></span><em>${esc(p.status)}</em></div>`).join(''):`<div class="season-good">No injury/status flags on your current roster.</div>`}</div></article>
        <article class="surface season-feature season-free-agents"><div class="season-card-head"><div><span class="section-label">WAIVER WIRE</span><h2>Best available players</h2></div><button class="btn secondary small" data-season-go="trades">Waivers & trades →</button></div><div class="season-list">${free.map((p,i)=>`<div><span class="season-rank">${i+1}</span><span><b>${esc(p.name)}</b><small>${esc(p.position||primaryPos(p))} · ${esc(p.team||'')} · ECR ${formatRank(p.rank)}</small></span><strong>${p.adjustedWeekProjection?`${p.adjustedWeekProjection.toFixed(1)} wk`:num(p.projection)!==null?Number(p.projection).toFixed(1):'—'}</strong></div>`).join('')||'<div class="season-good">No free-agent data available.</div>'}</div></article>
    ...
    bindActions(root);window.fmDerivedCache?.ensureRefreshControl?.('view-dashboard');renderFeedStatus?.();
  }
