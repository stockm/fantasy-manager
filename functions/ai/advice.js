...
function taskPrompt(task){
  ...
  return 'Act as the AI Fantasy GM. Analyze this exact NFL week, the fantasy opponent, adjusted projections, simulation and league rosters. Give the highest-impact actions first. Keep the response concise enough to complete all recommendations; do not start a section you cannot finish.';
}
...
async function callResponses(input,maxOutputTokens,previousResponseId=null){
  ...
  const response=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${OPENAI_API_KEY.value()}`,'Content-Type':'application/json'},body:JSON.stringify(body)});
  let data;try{data=await response.json()}catch(parseError){const raw=await response.text().catch(()=> '');console.error('OpenAI non-JSON response',response.status,raw.slice(0,1000));return{response,data:null,parseError:true}}
  return{response,data,parseError:false};
}

const aiAdvice=onRequest({secrets:[OPENAI_API_KEY],timeoutSeconds:90,memory:'256MiB'},async(req,res)=>{
  ...
  try{
    ...
    let advice=extractText(data),summary=responseSummary(data),continued=false;
    const hitOutputLimit=summary.status==='incomplete'&&/max_(?:output_)?tokens|max_tokens/i.test(String(summary.incompleteReason||''));
    if(advice&&hitOutputLimit&&data?.id){
      const second=await callResponses('Continue exactly where the previous answer stopped. Do not repeat earlier sections. Finish the current sentence and any remaining high-priority recommendations, then end with a short Bottom line. Keep this continuation under 1,500 words.',3200,data.id);
      if(!second.parseError&&second.response.ok){const continuation=extractText(second.data);if(continuation){advice=`${advice}\n\n${continuation}`;continued=true;summary=responseSummary(second.data)}}
    }
