...
// so test drafts cannot finish with missing QB/RB/WR/TE/FLEX/SFLEX/K/DST slots.
(function installTestAutoDraft(){
  ...
  function teamRequirementStatus(slot){
    ...
    const left={
      QB:Math.max(0,counts.QB-r.QB),
      RB:Math.max(0,counts.RB-r.RB),
      WR:Math.max(0,counts.WR-r.WR),
      TE:Math.max(0,counts.TE-r.TE)
    };
    ...
    const missing=[
      ...Object.entries(direct).flatMap(([x,n])=>Array(n).fill(x)),
      ...Array(flexMissing).fill('FLEX'),
      ...Array(sflexMissing).fill('SFLEX')
    ];
  ...
  function chooseAutoPlayer(slot,overall){
    ...
    if(status.missingCount>status.picksRemaining)return{player:null,status,error:`${leagueTeamName?.(slot)||`Team ${slot}`} no longer has enough picks to fill ${status.missingCount} required slots.`};

    let candidates=[];
    if(status.missing.length){
      // Required starters are filled before bench depth. K/DST are delayed until
      // the core offensive starter slots have been completed.
      const coreMissing=status.missing.filter(x=>x!=='K'&&x!=='DST');
      const targetSlots=coreMissing.length?coreMissing:status.missing;
      ...
      if(!candidates.length)return{player:null,status,error:`No available player can fill ${leagueTeamName?.(slot)||`Team ${slot}`}'s remaining required slots: ${status.missing.join(', ')}.`};
    }else{
      // Bench: prefer normal fantasy depth and avoid redundant K/DST. In 1-QB,
      // strongly de-emphasize a backup quarterback during test simulation.
  ...
  async function autoDraftToMyTurn(){
    ...
    if(made)toast(`${made} test picks simulated — you're on the clock at ${pickLabel(now.overall)}.`);
